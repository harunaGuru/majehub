...
import ChatInput from '@/app/shared/components/ChatInput';
...
const getConversations = async () => {
  const { data } = await axiosInstance.get('/chatting/api/conversations', isProtected());
...
const getMessages = async ({ queryKey }: any) => {
  ...
  const url = cursor
    ? `/chatting/api/conversations/${conversationId}/messages?cursor=${cursor}`
    : `/chatting/api/conversations/${conversationId}/messages`;

  const { data } = await axiosInstance.get(url, isProtected());
  return data;
};
...
const InboxPage = () => {
  ...
  // 🔥 websocket
  const wsRef = useChatSocket(user?.id, (data) => {
  ...
  const fetchMessages = async () => {
    const { data } = await axiosInstance.get(
      `/chatting/api/conversations/${selectedChat.conversationId}/messages`, isProtected()
    );
  ...
  return (
    <div className="bg-[#EFEEF1] h-fit py-4">
      <div className="w-[90%] lg:w-[80%] mx-auto ">
        <div className="flex flex-col lg:flex-row h-[90vh] rounded-2xl shadow-xl">
            ...
            <div className="overflow-y-auto flex-1">
              {conversations.map((chat: any) => (
                <div
                  ...
                  className={`flex gap-3 px-2 py-2 cursor-pointer hover:bg-gray-100 ${selectedChat?.conversationId === chat.conversationId && "bg-[#C5DFFA] hover:bg-[#C5DFFA]"}`}
                >
            ...
            {!selectedChat ? (
              ...
              <>
                ...
                <ChatInput
                  ...
                  senderId={user?.id}
                  receiverId={selectedChat?.seller?.id}
                  onMessageSent={(msg) => {
                    setMessages((prev) => {
